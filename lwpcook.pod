...
All examples should be runnable programs. You can in most cases
test the code sections by just piping the program text to perl.
...
the document specified by URL argument:
...
handle the the response returned.

  use LWP::UserAgent;
  $ua = new LWP::UserAgent;
  $ua->agent("$0/0.1");
  # $ua->agent("Mozilla/3.0") # pretend you are some new Netscape browser
...
library can also be used to posting data.
...
override the get_basic_credentials() method. Study the lwp-request
...
(There should of course be some LWP method that does all of this so
